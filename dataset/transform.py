...
class GroupOverSampleKaiming(object):
    ...
    def fill_fix_offset(self, image_w, image_h, crop_w, crop_h):
        ...
        if image_w == 256:
            ...
            raise ValueError(
                "Either image_w or image_h should be equal to 256")
...
class Stack(object):
    ...
    def __init__(self, mode="3D"):
        ...
        """
        assert (mode in ["3D", "TSN+2D", "2D", "TSN+3D"]
    ...
    def __call__(self, img_group):
        ...
        if "3D" in self.mode:
            imgs = np.concatenate([np.array(img)[np.newaxis, ...]
                                   for img in img_group], axis=0)
